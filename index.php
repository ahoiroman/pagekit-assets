        ...
        'site' => function ($event, $app) {
            $app->on('view.content', function ($event, $scripts) use ($app) {
                ...
                foreach ($assets = Asset::where(['status = ?'], [Asset::STATUS_PUBLISHED])->get() as $asset) {
                    if ((!$asset->nodes || in_array($app[ 'node' ]->id, $asset->nodes))) {
                        ...
                        if ($config[ 'caching' ] && $type != 'url') {
                            $hash                 =
                                substr(sha1($app->system()->config('secret') . date_format($asset->modified,
                                        'd/m/Y H:i:s')), 0, 4);
                        ...
                        if ($lang == 'js') {
                            if ($type == 'url' || $type == 'file') {
                                ...
                                if (!empty($asset->get('dependencies'))) {
                                    foreach ($asset->get('dependencies') as $key => $dependency) {
                                        if ($dependency === true) {
                                            $dependencies[] = $key;
                                        }
                                    }
                            ...
                            $app[ 'scripts' ]->add($slug, $content, $dependencies, $options);
                        } elseif ($lang == 'css') {
